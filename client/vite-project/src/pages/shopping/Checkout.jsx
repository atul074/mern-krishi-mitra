...
function ShoppingCheckout() {
  ...
  function handleInitiateCodOrder() {
    if (!cartItems || cartItems.items.length === 0) {
      alert("Your cart is empty. Please add items to proceed.");
      return;
    }
    ...
    const orderData = {
      ...
      orderStatus: "confirmed",  // directly confirmed for COD
      paymentMethod: "cod",
      paymentStatus: "pending",
      totalAmount: totalCartAmount,
      orderDate: new Date(),
    ...
    dispatch(createNewOrder(orderData)).then((data) => {
      if (data?.payload?.success) {
        ...
        window.location.href = "/shop/account"; // redirect to orders page
      } else {
        setIsPaymemntStart(false);
            ...
            <button
              onClick={() => {
                if (paymentMethod === "paypal") {
                  ...
                  handleInitiateCodOrder();
                }
              }}
              className={`w-full py-2 rounded-xl text-white ${isPaymentStart
